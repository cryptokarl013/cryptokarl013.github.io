---
title: Contact - CryptoKarl013
description: How to reach CryptoKarl013 for tips, corrections, right-of-reply requests, or collaboration on blockchain-forensics investigations.
permalink: /contact/
canonical_url: https://cryptokarl013.com/contact/
author: cryptokarl013
date: 2026-04-24
last_modified_at: 2026-04-24
---

# Contact

## Primary channel

**Email - cryptokarl013@gmail.com**

This is the preferred channel for:
- Tips and new evidence
- [Corrections](/corrections/) and right-of-reply requests
- Questions about methodology or data sources
- Requests for clarification of a specific report
- Collaboration on investigations

Email is preferred because it preserves the record and allows attachments (PDFs, JSON exports, screenshots).

## Alternate channels

These are all operated by the same author and are verified cross-links.

- **X (Twitter):** [@CryptoKarl013](https://x.com/CryptoKarl013)
- **Telegram:** [@cryptoinvestigations013](https://t.me/cryptoinvestigations013)
- **GitHub:** [@cryptokarl013](https://github.com/cryptokarl013)
- **Facebook:** [profile](https://www.facebook.com/profile.php?id=61577352392394)

## Response time

- Initial acknowledgement: within **3 business days** for most messages.
- Substantive reply (for [corrections](/corrections/) specifically): within **14 calendar days**.
- Unsolicited consulting / paid-engagement requests: see below.

## What this site does not do

- **Paid take-downs.** Requests to remove or modify a published report in exchange for payment are declined without further correspondence.
- **Commercial investigations for clients.** The research published here is self-directed. Paid investigative engagements are outside the scope of this site.
- **Recovery-service referrals.** The site does not endorse any crypto-asset "recovery" service. If you have lost funds, report the case to your local authorities and to platforms such as [Chainabuse](https://www.chainabuse.com/).
- **Pre-publication previews.** Reports are not shared in draft form outside of [right-of-reply outreach](/methodology/) to named subjects.

## Tips

If you have a specific tip that you would like investigated:

1. Prefer **transaction hashes and wallet addresses** over narrative - the site works from on-chain data, and a single tx hash is worth more than a paragraph of description.
2. If you cannot share evidence publicly, say so in the first message; the author will agree on a secure channel before you send anything sensitive.
3. Do not share **private keys, seed phrases, or login credentials**. The author does not need these and will not accept them.

## Optional support

If you find published reports useful and would like to support the cost of paid data sources and hosting, optional donations are accepted at the addresses below. **All published research is produced independently of whether any donation has been received** - donations are not solicited in individual reports, and no donor has influence over editorial decisions. See the [Editorial Standards](/editorial-standards/) for the conflict-of-interest policy.

- **ETH:** `0x913d978C2BEb68b8Deaa266e59894841d65cAbA4`
- **BTC:** `bc1qfe7gflhy76e45x52ajt9fhjlgw3nnndj04r9e4`
